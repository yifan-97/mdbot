...
var Botkit = {
  ...
  renderMessage: function (message) {
    // Logo display only for bot's messages
    if (message.type === 'incoming') {
      message.logo = "logo";
    }

    // Time is displayed once and only when it changes from previous value
    var last_time = Botkit.getCookie('botkit_msg_time');
    var message_time = new Date().toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
    if (!last_time) {
      Botkit.setCookie('botkit_msg_time', message_time, 1);
    }
    if ((message_time !== last_time) || (message.type === 'incoming')) {
